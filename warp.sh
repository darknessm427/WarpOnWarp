...
IFS=$'\n' readarray -t subnets_v4 < <(curl -s "https://raw.githubusercontent.com/darknessm427/WarpOnWarp/main/subnets_v4.txt" | grep -v '^#' | grep -v '^[[:space:]]*$' | cut -d '/' -f 1 | cut -d '.' -f 1-3)
...
cfwarpIP() {
	...
	if [[ -n $cpu ]]; then
		curl -L -o warpendpoint -# --retry 2 https://raw.githubusercontent.com/darknessm427/WarpOnWarp/main/endpoint/$cpu
	fi
}
